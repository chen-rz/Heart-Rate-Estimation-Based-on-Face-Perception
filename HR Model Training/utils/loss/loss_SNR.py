...
class SNR_loss(nn.Module):
    def __init__(self, clip_length=300, delta=3, loss_type=1, use_wave=False):
        ...
        # TODO self.bpm_range = torch.arange(self.low_bound, self.high_bound, dtype = torch.float).cuda()
        self.bpm_range = \
            torch.arange(self.low_bound, self.high_bound, dtype=torch.float).to(torch.device("cpu"))
        ...
        hanning = Variable(
            torch.from_numpy(
                np.hanning(self.time_length)).type(torch.FloatTensor),
            requires_grad=True
        ).view(1, -1)

        # TODO self.two_pi_n = two_pi_n.cuda()
        # TODO self.hanning = hanning.cuda()
        self.two_pi_n = two_pi_n.to(torch.device("cpu"))
        self.hanning = hanning.to(torch.device("cpu"))
    ...
    def forward(self, wave, gt, fps, pred=None, flag=None):  # all variable operation
        ...
        if self.loss_type == 1:
            ...
            # TODO norm_t = (torch.ones(batch_size).cuda() / torch.sum(complex_absolute, dim = 1))
            norm_t = \
                (torch.ones(batch_size).to(torch.device("cpu")) / torch.sum(complex_absolute, dim=1))
